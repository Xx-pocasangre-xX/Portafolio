    ...
    req.on('end', async () => {
      ...
      try {
        ...
        if (name.trim().length < 2)
          return json(res, 400, { error: 'El nombre debe tener al menos 2 caracteres.' });
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim()))
          return json(res, 400, { error: 'Ingresa un email válido.' });
        if (message.trim().length < 10)
          return json(res, 400, { error: 'El mensaje debe tener al menos 10 caracteres.' });
        ...
        if (error) {
          ...
          return json(res, 500, { error: 'Error al enviar el correo.' });
        }
...
server.listen(3001, () => {
  ...
  console.log('✓ RESEND_API_KEY:', process.env.RESEND_API_KEY ? 'cargada' : 'NO encontrada — revisa .env.local');
});
